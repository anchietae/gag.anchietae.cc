...
export const ItemData: [string, number, number, number?][] = [
	["Carrot", 0.275, 20, 100],
...
export const Rarity: [string, number, number][] = [
	["Normal", 1000, 1],
	["Gold", 10, 20],
	["Rainbow", 1, 50]
];
...
function getFruitData(name: string): [string, number, number, number?] | null {
	return ItemData.find(item => item[0] === name) || null;
}

function calculateVariant(variantName: string): number {
	const variant = Rarity.find(v => v[0] === variantName);
	return variant ? variant[2] : 1;
}

function calculateMutation(tool: Tool): number {
...
export function calculateFruit(tool: Tool): number {
